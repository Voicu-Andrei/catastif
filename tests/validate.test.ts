...
const CLIENT: ClientInput = {
  tip: 'persoana', nume: 'Ion', cui: null, nr_reg_com: null, cnp: null,
  adresa: null, judet: null, oras: null, cod_postal: null, telefon: null, email: null, note: null
}
const FURNIZOR: FurnizorInput = {
  nume: 'SC Profil SRL', cui: null, nr_reg_com: null, adresa: null,
  telefon: null, email: null, note: null
}
const LINIE: LinieComandaInput = {
  produs_id: null, descriere: 'Fereastră', cantitate: 1, unitate_masura: 'buc',
  cost_unitar: 100, pret_unitar: 200, cota_tva: 21, pozitie: 0
}
...
describe('validare la granița main', () => {
  it('respinge produse fără nume sau cu valori negative', () => {
    const bun = {
      nume: 'P', descriere: null, unitate_masura: 'buc', pret_referinta: null,
      cota_tva: 21, track_stock: false, stoc_curent: 0, prag_stoc: null, furnizor_id: null
    }
  ...
  it('respinge comenzi fără linii sau cu linii invalide', () => {
    ...
    expect(() => createComanda({ ...gol, linii: [{ ...LINIE, cantitate: 0 }] })).toThrow(/cantitatea/)
    expect(() => createComanda({ ...gol, linii: [{ ...LINIE, cantitate: NaN }] })).toThrow(/cantitatea/)
    expect(() => createComanda({ ...gol, linii: [{ ...LINIE, pret_unitar: -1 }] })).toThrow(/negativ/)
    expect(() => createComanda({ ...gol, linii: [{ ...LINIE, descriere: '', produs_id: null }] })).toThrow(/descriere/)
  })

  it('respinge achiziții cu dată invalidă', () => {
    expect(() =>
      createAchizitie({
        furnizor_id: null, data: 'ieri', numar_document: null, observatii: null,
        linii: [{ produs_id: null, descriere: 'X', cantitate: 1, unitate_masura: 'buc', cost_unitar: 1, data: 'ieri' }]
      })
    ).toThrow(/Data/)
  })
  ...
  it('furnizorul cu achiziții nu poate fi șters', () => {
    ...
    createAchizitie({
      furnizor_id: f.id, data: '2026-07-01', numar_document: null, observatii: null,
      linii: [{ produs_id: null, descriere: 'X', cantitate: 1, unitate_masura: 'buc', cost_unitar: 1, data: '2026-07-01' }]
    })
